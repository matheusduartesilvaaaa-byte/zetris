...
export function PayableTable() {
  ...
  const { data, isLoading } = usePayable({ pageSize: 50 });
  const createMutation = useCreatePayable();
  const payMutation = usePayPayable();
  const deleteMutation = useDeletePayable();

  async function handlePay(id: string) {
    const result = await payMutation.mutateAsync(id);
    if (result.success) {
      toast.success("Conta paga e lançada no fluxo de caixa");
    } else {
      toast.error(result.message ?? "Erro ao pagar conta");
    }
  }
                    ...
                    <tr key={account.id} className="border-b last:border-0 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-1">
                          {account.status !== "PAID" && (
                            <Button variant="ghost" size="icon" onClick={() => handlePay(account.id)}>
